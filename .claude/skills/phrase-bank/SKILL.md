...
**スペイン語とフランス語で狙う難易度が違う。取り違えないこと。**

| | スペイン語 | フランス語 |
|---|---|---|
| 狙い | **上級中心**（大学院・新聞レベル） | **初心者向けの日常会話** |
| レベル | C1・C2 が主。B2 まで含む | **A1〜B1 のみ。C1・C2 は書かない** |
| 分野 | 報道・時事 / 文学 / 学術・論文 / 上級口語 | 挨拶・買い物・交通・時間・体調・天気・家族 |
| 厚くする時制 | 点過去・接続法過去・接続法完了・条件法・分詞 | **直説法現在・複合過去・近接未来・命令法・半過去** |
| 避ける時制 | （偏らせないことだけ意識する） | 単純過去・大過去・前未来・接続法過去。接続法は `il faut que` `il vaut mieux que` `aimer que` 程度に留める |
| 語彙 | réfuter, corroborer のような硬い語も可 | 日常語だけ。硬い語は使わない |

フランス語で「直説法現在の比率を下げよう」としないこと。**初心者向けでは現在形が主役でよい。**
（かつて上級化したときの名残でそう書いてあったが、方針が変わっている。）

## フレーズを増やすときの方針

このアプリはフレーズが多いほど価値が上がる。**遠慮せず大量に足してよい。**
ただし単調な例文を並べるのではなく、次のような「教科書に出にくいが実際に必要なもの」を
意識して混ぜること（**上級のスペイン語向け**。フランス語は上の表に従う）。
