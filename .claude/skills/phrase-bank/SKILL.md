...
2. **トークンは重ねられない。** 熟語トークンの内側にある動詞を別トークンにすることはできない。
   動詞に活用ラベルを付けたいときは、熟語トークンが動詞を含まないように切ること。
   （例: 「Que yo sepa」を丸ごと熟語にすると sepa に活用ラベルが付けられない。
   `Que` と `yo` を skip にして `sepa` を独立させ、note に決まり文句だと書く）
