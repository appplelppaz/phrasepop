...
| 赤い波線 | **不規則活用**。語幹がどう変わるか、あるいはその時制の全人称を単語カードに出す |
...
ser    / 直説法現在 1人称単数 → 不規則形。この時制は soy / eres / es / somos / sois / son と変化する
...
なお **「規則どおりなら〜になるはず」という書き方はしない。** その語は規則形を取らないので、
存在しない形を示しても学習の役に立たない。語幹の変化として説明できるものはそう書き、
できないものはその時制の全人称を並べて、形そのものを覚えられるようにしている。
...
- `data/{lang}/verbs.json` … 活用表（スペイン語 156 語 / フランス語 159 語 × 各 11〜13 時制）＋不規則判定
...
現在のフレーズ数はスペイン語 120 件 / フランス語 120 件。A1 から C2 までを揃えてあり、
否定形・疑問形・熟語・ことわざ・形容詞の性数一致・口語表現に加えて、
新聞やニュース・論説・法律文・学術論文で実際に使われる C1〜C2 の語彙と構文
（無人称の se / on、譲歩の接続法、受動態、habida cuenta de・compte tenu de など）も入れてある。フレーズは多いほど価値が上がる設計なので、
スキルを使って足し続けてよい。
